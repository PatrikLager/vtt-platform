...
//  2. maps/cellar.json — the fixture the NEXT arc (line of sight) depends
//     on — actually loads AND actually has cover. goblin-ambush's failure
//     (spec §1.3) was a 32x32 field with nothing to hide behind; a map with
//     no blocking object would repeat it silently.
...
func TestTheDemoMapLoadsAndHasCover(t *testing.T) {
	m, err := mapdef.Load("../../maps/cellar.json")
	if err != nil {
