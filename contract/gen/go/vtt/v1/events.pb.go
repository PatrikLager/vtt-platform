...
// referenced so the log never depends on an art file being present at replay
...
type TileRef struct {
	...
	// art is one art id — a filename stem in the campaign's flat art/ directory
	// (2026-09-02-art-is-a-flat-library design spec §3.2). Empty means the
	// standard picture. It read "pack tile name" until Task 5 of that plan,
	// which deleted the map field naming a pack and refuses any map file that
	// still declares one.
	Art           string `protobuf:"bytes,3,opt,name=art,proto3" json:"art,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
type SceneCreated struct {
	...
	// needs its definition attached or it drifts: it means the art ids a shipped
	// map's overrides actually name. Exactly one shipped map overrides anything
	// — campaigns/example/maps/cellar.json, whose four TILE art ids are earth-1,
	// masonry-1, flagstone-1 and cellar-door. That path was maps/cellar/map.json
	// until 2026-09-02, when Task 5 of 2026-09-01-create-scene-leaves dissolved
	// the per-map directory. The definition USED TO read "the names in a pack
	// that mapdef.LoadPack loads and Resolve consults", naming cellar-basics as
	// the one pack a shipped map declared; no map declares a pack any more (Task
	// 5 of 2026-09-02-art-is-a-flat-library refuses one that does) and the four
	// ids are unchanged by the move, so the range is not.
	// client/public/std-pack is NOT in that set: it is a client-side rendering
	// manifest served as static files, never art the server resolves. Nor are
...
func (x *TokenRemoved) GetTokenId() string {
	if x != nil {
		return x.TokenId
	}
	return ""
...
func (x *Envelope) GetTokenRemoved() *TokenRemoved {
	if x != nil {
		if x, ok := x.Payload.(*Envelope_TokenRemoved); ok {
			return x.TokenRemoved
		}
	}
	return nil
}
...
type Envelope_TokenRemoved struct {
...
const file_vtt_v1_events_proto_rawDesc = "" +
	...
	"\rsession_ended\x18\x10 \x01(\v2\x14.vtt.v1.SessionEndedH\x00R\fsessionEnded\x128\n" +
