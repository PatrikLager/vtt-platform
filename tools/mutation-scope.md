...
| `internal/rules/conformance` | **10** | 0 | 48s | backlog, no reason on record |
...
Three packages — `internal/rules`, `internal/rules/conformance`,
`internal/adventure` — are outside the gate with **no recorded reason
anywhere**, not excluded on an argument but never added. That
...
**76** mutants across the table above sit in code no test reaches (40 + 4 + 0 +
...
Measured on the 2026-08-04 gate run: `internal/gateway` 5, `internal/campaign`
2, `internal/mcp` 1. `check-mutation.py` counts a timeout as a detection
...
Smallest first: `internal/rules/conformance` (10) → `internal/adventure` (14)
→ `internal/rules` (59).
