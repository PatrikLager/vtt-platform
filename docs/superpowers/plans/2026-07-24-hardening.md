...
**Interfaces:**
- Produces: `campaign.Append` stamps `env.SessionId` under `c.mu` before validation/persist: SessionStarted → fresh generated session id (crypto/rand hex, `sess-` prefix); all other events + Undo markers → currently-open session's id; no open session → **empty** (spec §1.1: "no open session → empty" — legitimate, e.g. out-of-session table setup; corrected 2026-07-24: this line originally read "left as-is (caller-supplied or empty)", silently diverging from the approved spec — workflow review caught it before merge; see `internal/campaign/campaign.go`'s `stampSessionID` and `session_stamp_test.go` case (d)/the closed-session sibling case for the fix and its behavioral-RED proof). Incoming NON-EMPTY session_id on non-SessionStarted events is OVERWRITTEN by the open session's id (campaign is authoritative), or CLEARED to "" if no session is open. `Undo(from, to int64, reason, eventID, actorRole, participantID string)` — sessionID param GONE.

- [ ] **Step 1: Behavioral RED.** Write `session_stamp_test.go` (package campaign_test) against CURRENT code — these tests compile and FAIL behaviorally (the strongest RED): (a) append SessionStarted with empty session_id → returned/broadcast envelope has non-empty `sess-`-prefixed id AND `State().Sessions[0].ID` equals it; (b) subsequent SceneCreated/TokenMoved envelopes carry that same session id; (c) after SessionEnded + new SessionStarted, later events carry the SECOND id; (d) event appended with NO open session gets its incoming session_id **cleared to empty** (even when caller supplies one) — per spec §1.1, corrected during workflow-level final review (was: "kept verbatim"); plus a sharper sibling case: open a session, END it, then append an event carrying the now-CLOSED session's own (previously valid) id → persisted/broadcast session_id must be EMPTY, not the stale closed id; (e) a non-SessionStarted event submitted with a WRONG non-empty session_id gets overwritten to the open session's; (f) Undo marker carries the open session's id. Run: `go test ./internal/campaign/ -run SessionStamp` — capture the behavioral failures (empty ids where non-empty expected; for (d), stale/wrong ids where empty is expected).
